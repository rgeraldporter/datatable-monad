...
import {DataPoint, DataTableConstructor, DataTableHeader, DataTableItem, DataTableMonad, DataTableRow, Monad} from './types';

describe('The DataTable monad', () => {
  it('should satisfy the first monad law of left identity', () => {
    const a: DataTableConstructor =
        [['Column 1', 'Column 2', 'Column 3'], [1, 2, 3], [2, 3, 4]];

    // more complex than usual as this is a typed monad; need a function that
    // adheres to type restrictions
    const f = (a: DataTableConstructor): DataTableMonad => DataTable.of(
        // @ts-ignore
        a.map(
  ...
  it('should satisfy the third monad law of associativity', () => {
    ...
    // 3. m = DataTable.of(a); m.chain(f).chain(g) ==== m.chain(x =>
    // f(x).chain(g))
    const associativityA = DataTable.of(a).chain(g).chain(f);
    const associativityB =
        DataTable.of(a).chain((x: DataTableConstructor) => g(x).chain(f));
  ...
  it('should be able to return the nth row by index', () => {
    const a: DataTableConstructor =
        [['Column 1', 'Column 2', 'Column 3'], [1, 2, 3], [2, 3, 4], [4, 4, 4]];
  ...
  it('should be able to prepend a row', () => {
    ...
    const a2: DataTableConstructor = [
      ['Column 1', 'Column 2', 'Column 3'], [0, 5, 9], [1, 2, 3], [2, 3, 4],
      [4, 4, 4]
    ];
  ...
  it('should be able to filter a table by rows', () => {
    ...
    const a2: DataTableConstructor =
        [['Column 1', 'Column 2', 'Column 3'], [2, 3, 4], [4, 4, 4]];
  ...
  it('should be able to filter a table by columns', () => {
    const a: DataTableConstructor =
        [['A', 'Column 2', 'Column 3'], [1, 2, 3], [2, 3, 4], [4, 4, 4]];

    const a2: DataTableConstructor =
        [['Column 2', 'Column 3'], [2, 3], [3, 4], [4, 4]];
  ...
  it('should be able to convert to an array of objects', () => {
    const a: DataTableConstructor =
        [['Col1', 'Column 2', 'Column 3'], [1, 2, 3], [2, 3, 4], [4, 4, 4]];
