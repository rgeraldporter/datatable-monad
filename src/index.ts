...
const filterColumns = (x: DataTableConstructor, f: Function) => {
    // @ts-ignores
    const headerList = getDataHeader(x).filter(f);
    const indices: number[] = getIndicesByHeader(getDataHeader(x), headerList);
    return [headerList].concat(
        // @ts-ignore
        getDataRows(x).map((b: DataTableRow) => filterRowByIndices(b, indices))
    );
};
...
const DataTable = (x: DataTableConstructor): DataTableMonad => ({
    ...
    filter: (f: Function): DataTableMonad =>
        // @ts-ignore
